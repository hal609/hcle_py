...
def main():
    ...
    while steps < n_steps:
        ...
        if steps % 1200 == 0 and len(scores) > 0:
            ...
            if episodes % 1 == 0:
                print('{} {} avg score {:.2f} total_steps {:.0f} fps {:.2f} games {} eps {:.2f}'
                      .format(agent_name, game, avg_score, steps, (steps - last_steps) / (time.time() - last_time), episodes, agent.epsilon.eps),
                      flush=True)
                last_steps = steps
                last_time = time.time()
